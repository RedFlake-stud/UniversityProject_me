...
class SelectOptionPage(BasePage):
    def __init__(self, parent, controller, manager):
        ...
        label = tk.Label(self, text="Choose Hospital from list:", font=('Arial', 24))
        label.pack(pady=20)
        ...
        self.buttonframe.rowconfigure(0, weight=3)

        add_btn = tk.Button(self.buttonframe, text="Add Items", height=6, font=('Arial', 20),
                               command=self.go_to_add_page)
        add_btn.grid(row=0, column=0, sticky="ew")

        remove_btn = tk.Button(self.buttonframe, text="Remove Items", height=6, font=('Arial', 20),
